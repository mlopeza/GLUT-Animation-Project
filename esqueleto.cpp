...
GLfloat matAmbient [] =  {0.329412,0.223529,0.027451,   //Material: Brass
	0.0215, 0.1745, 0.0215,		//Material: Emmerald
	0.135, 0.2225, 0.1575,		//Material: Jade
	0.25,0.20725,0.20725,			//Material: Pearl
	0.2125,0.1275,0.054,			//Material: Bronze
	0.24725,0.1995,0.0745,		//Material: Gold
	0.19225,0.19225,0.19225,		//Material: Silver
	0.1,0.18725,0.1745};			//Material: Turquoise
...
//Manejo de texturas
GLuint	texture[5],xt,yt,zt;
int texAct; //Variable que maneja el cambio de textura del vestuario

void cargaImagenes(){
	//Lectura y Carga de Imagenes en el Arreglo
	texture[0] = LoadTexture("textu/circus.tga");
	texture[1] = LoadTexture("textu/dot_texture.tga");
	texture[2] = LoadTexture("textu/dots.tga");
	texture[3] = LoadTexture("textu/clown31.tga");
	texture[4] = LoadTexture("textu/floor.tga");
}
...
/*Metodo que permite crear la escena donde se encontrará
* situado el personaje
*/
void dibujaEscena(){
	// construcción de la escena
	// dibuja un espacio para el personaje
	glBindTexture(GL_TEXTURE_2D,texture[0]); // Se mapea la textura actual
	glBegin( GL_QUADS ); //lateral derecha
	glTexCoord2f(1.0f,1.0f); glVertex3f( 8, -9, -5);
	glTexCoord2f(0.0f,1.0f); glVertex3f( 8, -9, 10);
	glTexCoord2f(0.0f,0.0f); glVertex3f( 8, 9, 10);
	glTexCoord2f(1.0f,0.0f); glVertex3f( 8, 9, -5);
	glEnd();

	glBindTexture(GL_TEXTURE_2D,texture[0]); // Se mapea la textura actual
	glBegin( GL_QUADS ); // lat izq
	glTexCoord2f(0.0f,0.0f); glVertex3f(-8, 9, -5);
	glTexCoord2f(1.0f,0.0f); glVertex3f( -8, 9, 10);
	glTexCoord2f(1.0f,1.0f); glVertex3f( -8, -9, 10);
	glTexCoord2f(0.0f,1.0f); glVertex3f( -8, -9, -5);
	glEnd();
...
void renderScene(void) {
	...
	dibujaEscena(); //dibuja escena (Circo)
...
int dibuja(Elemento *nodo){
	...
	if(strcmp(nodo->type,"cube") == 0){
		...
		if((strcmp(nodo->name, "PD1") == 0) || (strcmp(nodo->name, "PI1") == 0) || (strcmp(nodo->name, "BD1") == 0) || (strcmp(nodo->name, "BI1") == 0)){
			glEnable(GL_TEXTURE_2D);
			glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_DECAL);
			gluQuadricTexture(nodo->q, true);
			glBindTexture(GL_TEXTURE_2D, texture[texAct]);
		...
		if((strcmp(nodo->name, "Cabeza") == 0)){
			...
			if((strcmp(nodo->name, "T->BD1") == 0) || (strcmp(nodo->name, "T->BI1") == 0) || (strcmp(nodo->name, "T->PI1") == 0) || (strcmp(nodo->name, "T->PD1") == 0)){
...
void processNormalKeys(unsigned char key, int xx, int yy) { 
	...
	case 'h':
		break;
	case 'j':
			texAct = (texAct == 1)? 2 : 1; //Cambiamos de textura el personaje
		break;
	case 'k':    
		break;
